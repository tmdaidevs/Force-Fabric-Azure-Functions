...
def _make_tool_handler(tool_def):
    """Factory that creates a properly-closed handler for each tool."""
...
for _tool in all_tools:
    if _tool["name"] in AUTH_TOOL_NAMES:
        continue
    _handler = _make_tool_handler(_tool)
    mcp.tool(name=_tool["name"], description=_tool["description"])(_handler)
